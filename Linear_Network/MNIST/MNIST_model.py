...
class ConvNet(nn.Module):
    def __init__(self,outputs,image_shape):
        ...
        self.fc1 = nn.Sequential(nn.Linear(units, units))
        self.fc2 = nn.Sequential(nn.Linear(units, units))
        self.fc3 = nn.Sequential(nn.Linear(units, units))
        self.fc4 = nn.Sequential(nn.Linear(units, outputs))
        print("fc parameters: ",sum(p.numel() for p in self.fc4.parameters()))
